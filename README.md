...
### 1. 설계 차이로 인한 동기화 지연 (Time-Lag)
* **E-Gen (공식 포털):** 일선 병원이 국립중앙의료원의 국가 응급의료 전산망인 **'중앙응급의료상황판(K-응급상황정보)'** 내부망에 입력한 데이터가 다이렉트로 반영되는 실시간(Real-Time) 구조입니다.
* **공공데이터 Open API:** 정부 내부망 보안(망분리) 및 서버 부하 방지를 위해 운영 DB에 직접 질의하지 못하며, 공공데이터 연계 서버의 배치(Batch) 주기와 캐시 레이어를 거쳐 비동기로 미러링됩니다.
* **현상:** 공식 웹 대비 수 분에서 십수 분의 전송 딜레이가 상시 발생합니다. 1~2분 단위로 환자 입·퇴원 및 응급 이송이 일어나는 응급실 특성상, 공식 화면과 애플리케이션 표출 수치 간 일시적 싱크 불일치가 불가피합니다.

### 2. 특정 데이터 Null 값 및 집계 기준 불일치
* **특수 병상(코호트 격리 등) 데이터 누락:** API 명세(XML 스키마)상에는 관련 필드(`hv42` 등)가 정의되어 있으나, 실제 연계 파이프라인에서 값이 유실되어 페이로드에 `null` 또는 빈 태그로 송출됩니다. 이에 따라 해당 항목은 정보 없음(`-`)으로 표출됩니다.
* **병상 집계 모수(범위)의 차이:**
  * **E-Gen 포털:** 병원 전체의 입원 격리병동 및 권역외상센터 병상까지 통합 집계하여 표출하는 경우가 존재합니다.
  * **Open API:** 응급실(ER) 내부 격리 구역만을 기준으로 수집합니다.
  * 동일 의료기관을 조회하더라도 기준 병상(분모)과 잔여 병상(분자) 수치에 차이가 발생할 수 있습니다.

### 3. 대응 전략
* **데이터 안정성 기반 대체 구성:** 결측 빈도가 높은 '코호트 격리' 대신, 수신 신뢰도가 높은 기존의 **'응급실'** 위주로 가용 현황 등을 UI로 대체 구성하여 사용자에게 실효성 있는 정보를 제공하는 것으로 협의.
* **면책 고지 및 유선 확인 유도:** API 자체의 갱신 지연은 클라이언트에서 해결할 수 없는 원천적 한계이므로, 서비스 화면 내 면책 문구를 명시하고 즉각적인 전화 연결 액션을 유도합니다.

> **[안내]** 본 서비스의 병상 정보는 공공데이터 연계 주기에 따라 실제 병원 현장 및 중앙응급의료센터(E-Gen) 공식 현황과 시차가 발생할 수 있습니다. 응급 상황 시 반드시 해당 기관에 유선 확인 후 방문하시기 바랍니다.